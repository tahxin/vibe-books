...
const ReadBooksList = () => {
  ...
  const currentList = activeTab === 'read' ? readBooks : wishlist;
  ...
  const sortedBooks = useMemo(() => {
    ...
    if (sortBy === 'totalPages') {
      return list.sort((a, b) => b.totalPages - a.totalPages);
    }
    if (sortBy === 'yearOfPublishing') {
      return list.sort((a, b) => b.yearOfPublishing - a.yearOfPublishing);
    }
    return list;
  }, [currentList, sortBy]);
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-6xl">
      ...
      {/* Tabs */}
      <div className="flex border-b border-base-200 mb-8 gap-2">
        <button
          onClick={() => setActiveTab('read')}
          className={`py-3 px-5 font-semibold text-base transition-colors border-b-2 -mb-[2px] flex items-center gap-2 ${
            activeTab === 'read'
        ...
        >
          ...
          <span className="badge badge-sm badge-neutral">{readBooks.length}</span>
        </button>
        ...
        >
          ...
          <span className="badge badge-sm badge-neutral">{wishlist.length}</span>
        </button>
      </div>
      ...
      {sortedBooks.length === 0 ? (
        <div className="text-center py-16 px-4 bg-base-200/30 rounded-2xl border border-dashed border-base-300">
          ...
          <h2 className="text-xl font-bold text-base-content">
            No books in {activeTab === 'read' ? 'Read List' : 'Wishlist'} yet
          </h2>
          <p className="text-sm text-base-content/70 mt-2 max-w-md mx-auto">
            Discover our collection of books and add them to your {activeTab === 'read' ? 'Read List' : 'Wishlist'}!
          </p>
